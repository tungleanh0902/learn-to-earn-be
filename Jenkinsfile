                    ...
                    if (target == 'develop') {
                        ...
                        sshagent(['0a39231d-d882-4824-ae7f-0d892c489685']) {
                                sh """ssh -o StrictHostKeyChecking=no builder@206.189.158.254 << EOF
                                    ...
                                    docker build -t registry.gitlab.com/eupsolution/l2e-api:${GIT_COMMIT} .
                                    docker push registry.gitlab.com/eupsolution/l2e-api:${GIT_COMMIT}
                                    docker rmi registry.gitlab.com/eupsolution/l2e-api:${GIT_COMMIT}
                                exit
                            EOF"""
                        }
                    }
                    ...
                    if (target == 'develop') {
                        ...
                        sshagent(['0a39231d-d882-4824-ae7f-0d892c489685']) {
                                sh """ssh -o StrictHostKeyChecking=no mazii@157.245.203.48 << EOF
                                    ...
                                    git pull origin deployment/dev
                                    docker pull registry.gitlab.com/eupsolutionl2e-api:${GIT_COMMIT}
                                    exit
                                EOF"""
